...
export async function POST(request: NextRequest) {
  ...
  if (sessionType === "free_practice") {
    systemPrompt = `אתה שחקן ערבי דובר ערבית בלבד. הסיטואציה: ${description ?? "שיחה יומיומית"}. פתח את השיחה והמשך בערבית בלבד. השב בצורה קצרה וטבעית. אל תתקן את בן שיחתך.`;
  } else {
    systemPrompt = `אתה ערבי מקומי מנהל שיחה יומיומית טבעית. דבר ערבית בלבד (ניב לבנטיני / פלסטיני). שיחה ידידותית ופתוחה. השב בצורה קצרה וטבעית.`;
  }
